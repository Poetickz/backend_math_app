...
# if 'valor' in request.json and 'fecha' in request.json and 'id_visita' in request.json and 'id_beacon' in request.json and 'unidad' in request.json:
#     db_connection = db()
#     #fechaInicio = datetime.strptime(request.json['inicio'], '%Y/%m/%d %H:%M:%S')
#     params = [float(request.json['valor']), request.json['fecha'], int(request.json['id_visita']), int(request.json['id_beacon']), request.json['unidad']]
#     print(params)
#     #print(params)
#     sp = 'CALL insertMedicion (?, ?, ?, ?, ?)'
#     value = db_connection.insertSP(params, sp)
#     db_connection.disconnect()
#     response = jsonify(status = 'Success', id = value)
    # addHeaders(response)
#     return response
# else:
#     return jsonify(status = 'Failed')
